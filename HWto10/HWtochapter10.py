...
# Convert 'data' from its original format into a NumPy array.
data = np.array(data)

# Normalize the data
data_normalized = z_score_normalize(data[:, 1:])  # Assuming the first column is labels
data = np.hstack((data[:, 0:1], data_normalized))  # Re-attach the labels
